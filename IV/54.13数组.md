...
newarray指令，创建了一个有10个整数元素的数组，数组的大于设置使用bipush指令，然后结果返回到栈顶。数组类型用newarry指令操作符进行设定。

932

呢哇仍然有被执行后，引用（指针）到新创建的数据，栈顶的槽中，astore_1存储引用指向到LVA的一号槽，main()函数的第二个部分，是循环的存储值1到相应的素组元素。
...
53.13.8总结
在java中，可能出现栈移除吗？不能，因为素组的长度能代表数组对象，数组的边界是可控的，而越界访问的情况会被抛出异常。
