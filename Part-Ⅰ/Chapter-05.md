...
`_snprintf()`函数作用与`printf()`函数基本相同,不同的地方是`printf()`会将结果输出到的标准输出中(例如：终端和控制台),`_snprintf()`会将结果保存到缓冲区中,`puts()`会将缓冲区的内容复制到标准输出。当然，后面两⾏代码可以使只用一个`printf()`调用替换,但我们必须说明小缓冲区的用途。
